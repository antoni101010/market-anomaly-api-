...
def valuation_score(metrics: dict) -> float | None:
    """
...
def quality_from_metrics(metrics: dict) -> float | None:
    """
...
def dilution_risk_score(
    ...
    if share_growth is None and cash_runway is None and fcf_margin is None:
        return None
...
def distress_risk_score(
    ...
    # Le soglie dimensionali sono in USD. Una capitalizzazione espressa in
    # yen/euro/dollari locali non viene confrontata direttamente con tali
    # soglie: il provider deve fornire la conversione esplicita.
    market_cap = safe(
    ...
    risk = risk if risk is not None else 25.0
...
def value_trap_risk(
    ...
    if not any(value is not None for value in available_fundamental_signals):
        return None
...
def completeness_details(metrics: dict) -> dict:
    groups = {
        "valuation": [
            "pe_ratio", "forward_pe", "ev_to_ebitda", "ev_to_sales",
            "price_to_sales", "price_to_book", "fcf_yield_pct",
        ],
        "growth_profitability": [
            "revenue_growth_pct", "eps_growth_pct", "net_margin_pct",
            "operating_margin_pct", "fcf_margin_pct",
        ],
        "balance_sheet": [
            "debt_to_ebitda", "current_ratio", "interest_coverage",
            "liabilities_to_assets", "cash_runway_months",
        ],
        "dilution": ["shares_outstanding_growth_pct"],
    }
...
def enrich_fundamental_scores(
    ...
    if safe(result.get("pe_ratio")) is None:
        result["pe_ratio"] = safe(result.get("approx_pe"))

    if safe(result.get("price_to_sales")) is None:
        result["price_to_sales"] = safe(result.get("approx_ps"))
    ...
    completeness = completeness_details(result)
    ...
    result.update({
        ...
        "missing_fundamental_fields": completeness["missing_fields"],
    })
