...
@app.post("/api/scan")
def trigger_scan(
    limit: int = Query(300, ge=25, le=500),
    catalyst_top_n: int = Query(120, ge=1, le=200),
    x_api_key: str | None = Header(default=None),
):
    _check_api_key(x_api_key)

    return service.start_scan_background(
        limit=min(limit, max(300, int(CONFIG.deep_candidate_limit))),
...
def dashboard(
    ...
    max_value_trap: float = Query(100, ge=0, le=100),
    min_anomaly: float = Query(60, ge=0, le=100),
    max_anomaly: float = Query(100, ge=0, le=100),
