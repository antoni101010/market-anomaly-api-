...
def test_market_tension_uses_full_light_universe_for_global_breadth():
    broad = pd.DataFrame({
        "light_ema200_pct": [15.0] * 80 + [-15.0] * 20,
        "light_ema50_pct": [8.0] * 75 + [-8.0] * 25,
        "light_drawdown_250d_pct": [-5.0] * 70 + [-20.0] * 20 + [-40.0] * 10,
    })
    result = calculate_market_tension(
        _valuation_rows(high=True),
        {},
        expected_valuation_rows=9,
        broad_market_rows=broad,
        source="synthetic-test",
    )
    assert result["global_breadth"]["companies"] == 100
    assert result["global_breadth"]["above_200d_pct"] == 80.0
    assert result["price_euphoria"] is not None
    assert result["fragility"] is not None
    assert result["methodology_version"] == "market-tension-3.0"
